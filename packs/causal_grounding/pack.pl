...
version('0.1.0').
% State the fact: title naming the pack as the shared minting helpers every grounded construct reuses.
title('Connectome atomic — causal_grounding: the shared Causalontology 3.0.0 minting vocabulary').
...
requires([]).
% State the fact: layer(0) — the minting vocabulary is substrate: every grounded construct imports it.
layer(0).
