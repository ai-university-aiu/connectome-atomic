% State the fact: name(thalamus) — the relay that closes the loop, region and structure in one pack.
name(thalamus).
% State the fact: version('0.1.0') — a granularity-experiment prototype.
version('0.1.0').
% State the fact: title naming the thalamus and its relay / loop-closing role plus its structure record.
title('Connectome atomic — thalamus: the relay that closes the loop and carries the cortisol event').
