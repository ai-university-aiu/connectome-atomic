...
# Region behaviour is proven end-to-end by bin/run_slice.sh (the closure verdict
# is its real test); these suites cover the pure, structural and interface packs
# and confirm every module loads. Exit 0 iff all suites pass.
set -u
...
# The regions and neural_lattice need PrologAI's lattice and actors packs.
LIB="$LIB -p library=$PROLOGAI_HOME/packs/lattice/prolog"
LIB="$LIB -p library=$PROLOGAI_HOME/packs/actors/prolog"
# The grounded packs need the Causalontology engine and conformance harness.
