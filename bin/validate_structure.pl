/*  Connectome atomic — the Causalontology 2.0.0 structure validator.
    ...
    A MEASURED CONSEQUENCE (ATOMIC-4). Because each region co-locates its
    continuant record WITH its runtime, importing the region packs drags in the
    Lattice substrate: validating STRUCTURE now loads the whole RUNTIME stack.
    In the slice, causal_map imported no Lattice, so structure validation was
    cleanly separable. Atomic co-location couples the two (see LEDGER.md).
*/

% Import the nine construct packs that each mint their own structure records.
...
% -- validate_structure_records(-Records): assemble the twenty-eight records from the nine packs.
validate_structure_records(Records) :-
    % Collect each construct pack's own record list.
    neuroendocrine_strata_records(RStrata),
    dopamine_records(RDopamine),
    cortisol_records(RCortisol),
    plasticity_records(RPlasticity),
    cortex_records(RCortex),
    striatum_records(RStriatum),
    thalamus_records(RThalamus),
    corticostriatal_conduit_records(RCortico),
    nigrostriatal_conduit_records(RNigro),
    % Concatenate them, grouped by owning construct (order does not affect ids or validity).
    append([RStrata, RDopamine, RCortisol, RPlasticity, RCortex, RStriatum,
            RThalamus, RCortico, RNigro], Records).
...
validate_skip(Result) :-
    % Classify the cortisol CRO and read its skip-gaps (from the cortisol pack).
...
validate_signature(Result) :-
    % Fetch the signed assertion over the skip CRO (from the cortisol pack).
