/*  Connectome atomic — the runner (control arm of the Wave 3 granularity experiment).

    Ticks the reentrant cortico-basal-ganglia-thalamo-cortical loop for N laps
    and prints the narrated, glass-box trace. The BEHAVIOUR is held constant with
    the Wave 2 slice — only the pack boundaries differ (one pack per named
    construct). Each region runs as a PrologAI cyclic_actor (a background thread);
    the regions coordinate ONLY through the Lattice, via numbered phase cues and
    lattice_await/notify — stigmergy for state, notification for reactivity, zero
    actor-to-actor references, no busy-poll. When the loop comes to rest the
    runner PROVES it closed: the right number of hops, the exact region sequence,
    a strictly monotonic token, and the cortex re-entered on every lap. Exit 0 on
    a clean, proven run.
...
% Import the three region packs for their tick predicates (one pack per region).
...
run_slice(NLaps, EventLap, Rate) :-
    ...
    % Spawn the three region threads; each blocks on its own phase cue (no busy-poll).
    cyclic_actor(cortex,   cortex:cortex_tick(Nexus),     0),
    cyclic_actor(striatum, striatum:striatum_tick(Nexus), 0),
    cyclic_actor(thalamus, thalamus:thalamus_tick(Nexus), 0),
...
run_slice_banner(NLaps, EventLap, Rate) :-
    ...
    format("~n== connectome-atomic :: one pack per named construct (Wave 3a control) ==~n", []),
    % Print the loop being ticked.
    format("Loop: cortex -> striatum -> thalamus -> cortex (reentrant)~n", []),
...
run_slice_report(NLaps, Outcome, ExpectedHops, Hops, Verdict) :-
    ...
    format("  actor-to-actor refs: 0 (regions share only the Lattice; verified by grep in bin/check_no_coupling.sh)~n", []),
