...
# (library(layer), Ledger entry L4) against THIS arm's eleven packs.
#
# The atomic arm has the MOST packs of the three granularity arms, so it is the
# stiffest test of the layer construct. Every pack declares layer(N) in its
# pack.pl; this checker loads PrologAI's library(layer) by its real path (no
# copy, no modification) and calls its directory-scoped report + check against
# the arm's packs/ dir, reusing the SAME construct through its public
# layer_report_dir/1 and layer_check_dir/2 predicates — reuse, not reimplement.
#
